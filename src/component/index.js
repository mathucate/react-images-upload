...
class ReactImageUploadComponent extends React.Component {
  ...
  moveImage(dragIndex, hoverIndex) {
    const dragPicture = this.state.pictures[dragIndex];
    const dragFile = this.state.files[dragIndex];
    const reorderedPictures = update(this.state.pictures, {
      $splice: [
        [dragIndex, 1],
        [hoverIndex, 0, dragPicture],
      ],
    });
    const reorderedFiles = update(this.state.files, {
      $splice: [
        [dragIndex, 1],
      ...
      return (
        <div key={index} className='uploadPictureContainer'>
          <PreviewPicture
            index={index}
            picture={picture}
            removeImage={() => this.removeImage(picture)}
            moveImage={(a, b) => this.moveImage(a, b)}
          />
        </div>
      );
    });
